import { Canvas, Meta, Story } from '@storybook/addon-docs/blocks';

<Meta title="UI/Navigation/Breadcrumbs" />

# Breadcrumbs

A list of links charting the user’s place in the site structure. The title of the current page should be in an `<h1>` on the page, so it isn’t needed here.

<Canvas isColumn>
  <Story name="Breadcrumbs">
    {`
      <ol class="c-breadcrumbs u-h6 u-list-reset u-flex u-flex--wrap u-items-center">
        <li class="u-margin-xs--right u-flex u-items-center">
          <a href="/">Main section</a>
        </li>
        <li class="u-margin-xs--right u-flex u-items-center">
          <svg class="a-icon a-icon--m u-fg--gray-30 u-margin-xs--right" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
            <path d="M32.83,97.22a6.07,6.07,0,1,1-8.59-8.58L58.59,54.29a6.07,6.07,0,0,0,0-8.58L24.24,11.36a6.07,6.07,0,1,1,8.59-8.58L75.76,45.71a6.07,6.07,0,0,1,0,8.58Z" fill-rule="evenodd"/>
          </svg>
          <a href="/">Sub section</a>
        </li>
        <li class="u-margin-xs--right u-flex u-items-center">
          <svg class="a-icon a-icon--m u-fg--gray-30 u-margin-xs--right" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" aria-hidden="true">
            <path d="M32.83,97.22a6.07,6.07,0,1,1-8.59-8.58L58.59,54.29a6.07,6.07,0,0,0,0-8.58L24.24,11.36a6.07,6.07,0,1,1,8.59-8.58L75.76,45.71a6.07,6.07,0,0,1,0,8.58Z" fill-rule="evenodd"/>
          </svg>
          <a href="/">Sub page</a>
        </li>
      </ol>
    `}
  </Story>
</Canvas>

Using the same structure, your root page might be represented with an icon, to make it really quick to spot.

<Canvas>
  <Story name="Breadcrumbs with icon">
    {`
      <ol class="c-breadcrumbs u-h6 u-list-reset u-flex u-flex--wrap u-items-center u-line-height--min">
        <li class="u-margin-xs--right u-flex u-items-center">
          <a href="/" class="u-margin-xs--right u-flex u-items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="a-icon a-icon--l" viewBox="0 0 100 100">
              <path d="M93.79,98H64.38a4.17,4.17,0,0,1-4.17-4.17V66.68H39.79V93.79A4.17,4.17,0,0,1,35.66,98H6.21A4.17,4.17,0,0,1,2,93.87V43.79a4.18,4.18,0,0,1,1.5-3.21L47.29,3a4.17,4.17,0,0,1,5.43,0L96.5,40.58A4.26,4.26,0,0,1,98,43.79V93.74A4.17,4.17,0,0,1,93.92,98Z"/>
            </svg>
            <span class="u-sr-only">Home</span>
          </a>
        </li>
        <li class="u-margin-xs--right u-flex u-items-center">
          <svg class="a-icon u-fg--gray-30 u-margin-xs--right" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
            <path d="M32.83,97.22a6.07,6.07,0,1,1-8.59-8.58L58.59,54.29a6.07,6.07,0,0,0,0-8.58L24.24,11.36a6.07,6.07,0,1,1,8.59-8.58L75.76,45.71a6.07,6.07,0,0,1,0,8.58Z" fill-rule="evenodd"/>
          </svg>
          <a href="/">Sub section</a>
        </li>
        <li class="u-margin-xs--right u-flex u-items-center">
          <svg class="a-icon a-icon--m u-fg--gray-30 u-margin-xs--right" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" aria-hidden="true">
            <path d="M32.83,97.22a6.07,6.07,0,1,1-8.59-8.58L58.59,54.29a6.07,6.07,0,0,0,0-8.58L24.24,11.36a6.07,6.07,0,1,1,8.59-8.58L75.76,45.71a6.07,6.07,0,0,1,0,8.58Z" fill-rule="evenodd"/>
          </svg>
          <a href="/">Sub page</a>
        </li>
      </ol>
    `}
  </Story>
</Canvas>
