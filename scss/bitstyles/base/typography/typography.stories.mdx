...
The default configuration imports the styles set in [`settings/responsive-typography`](/docs/design-tokens-typography--page), so all font-size configuration should happen there. This ensures these base styles match the classes specified in `utilities/typography-responsive`.
