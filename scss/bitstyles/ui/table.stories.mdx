import { Canvas, Meta, Story } from '@storybook/addon-docs/blocks';

<Meta title="UI/Data/Table" />

# Table

Add base styling to your tables by giving them the `o-table` class. As shown here, always add an overflow wrapper to handle smaller viewports. Tables do not resize well, so allowing horizontal scrolling is a simple and clear way to make the table accessible to smaller devices.

<details class="u-margin-xl--y">
  <summary class="u-h4 u-font--medium">Examples use</summary>
  <ul>
    <li><a href="/?path=/story/atoms-badge--badge">a-badge</a></li>
    <li><a href="/?path=/story/atoms-button-base--button-element">a-button</a></li>
    <li><a href="/?path=/story/atoms-button-danger--danger">a-button--danger</a></li>
    <li><a href="/?path=/story/atoms-button-small--small">a-button--small</a></li>
    <li><a href="/?path=/story/atoms-button-ui--ui">a-button--ui</a></li>
    <li><a href="/?path=/story/atoms-card--card">a-card</a></li>
    <li><a href="/?path=/story/atoms-list-reset--list-reset">a-list-reset</a></li>
    <li><a href="/?path=/story/organisms-table--table">o-table</a></li>
    <li><a href="/?path=/story/utilities-flex--flex">u-flex</a></li>
    <li><a href="/?path=/story/utilities-typography--typography">u-h6</a></li>
    <li><a href="/?path=/story/utilities-margin--margin-m">u-margin</a></li>
    <li><a href="/?path=/story/utilities-overflow--overflow-x">u-overflow</a></li>
    <li><a href="/?path=/story/utilities-padding--padding-m">u-padding</a></li>
    <li><a href="/?path=/story/utilities-sr-only--sr-only">u-sr-only</a></li>
    <li><a href="/?path=/story/utilities-text--text-right">u-text</a></li>
  </ul>
</details>

<Canvas>
  <Story name="Table">
    {`
      <div class="a-card u-padding-0 u-overflow--x">
        <table class="o-table u-h6">
          <caption class="u-sr-only">Describe the table content here</caption>
          <thead>
            <tr>
              <th scope="col">Name</th>
              <th scope="col">Email</th>
              <th scope="col">Status</th>
              <th scope="col"><span class="u-sr-only">Actions</span></th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>Sesame S. Snaps</td>
              <td>sesame.snaps@gmail.com</td>
              <td><span class="a-badge a-badge--danger">Banned</span></td>
              <td class="o-table__actions u-text--right">
                <ul class="a-list-reset u-flex u-flex--inline">
                  <li class="u-margin-xs--right">
                    <a href="/" class="a-button a-button--ui a-button--small">Show</a>
                  </li>
                  <li class="u-margin-xs--right">
                    <a href="/" class="a-button a-button--ui a-button--small">Edit</a>
                  </li>
                  <li>
                    <a href="/" class="a-button a-button--ui a-button--danger a-button--small">Delete</a>
                  </li>
                </ul>
              </td>
            </tr>
            <tr>
              <td>Croissant B. Gummies with the very long multi-word name</td>
              <td>croissant.gummies@gmx.de</td>
              <td><span class="a-badge a-badge--positive">Confirmed</span></td>
              <td class="o-table__actions u-text--right">
                <ul class="a-list-reset u-flex u-flex--inline">
                  <li class="u-margin-xs--right">
                    <a href="/" class="a-button a-button--ui a-button--small">Show</a>
                  </li>
                  <li class="u-margin-xs--right">
                    <a href="/" class="a-button a-button--ui a-button--small">Edit</a>
                  </li>
                  <li>
                    <a href="/" class="a-button a-button--ui a-button--danger a-button--small">Delete</a>
                  </li>
                </ul>
              </td>
            </tr>
            <tr>
              <td>Liquorice G Fruitcake</td>
              <td>liquorice.fruitcake@mailbox.org</td>
              <td><span class="a-badge a-badge--positive">Confirmed</span></td>
              <td class="o-table__actions u-text--right">
                <ul class="a-list-reset u-flex u-flex--inline">
                  <li class="u-margin-xs--right">
                    <a href="/" class="a-button a-button--ui a-button--small">Show</a>
                  </li>
                  <li class="u-margin-xs--right">
                    <a href="/" class="a-button a-button--ui a-button--small">Edit</a>
                  </li>
                  <li>
                    <a href="/" class="a-button a-button--ui a-button--danger a-button--small">Delete</a>
                  </li>
                </ul>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    `}
  </Story>
</Canvas>
