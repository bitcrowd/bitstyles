...
- **round** A button with rounded corners, which matches the size of other buttons at a given size. Combine with **square** to make a circular button (useful for circular icon buttons).
...
- **Transparent** Use `outline` buttons to de-emphasize an action (see the “cancel” buttons in the examples up top). Use also for nav menus, sidebars, and dropdown menus.
- **Dangerous** A special case that only applies its appearance on hover and active. Use these to indicate an action the user should be wary of, most often something destructive to data. This variant can be combined with all other button color variants.
