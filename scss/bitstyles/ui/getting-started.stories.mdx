...
If you’re configuring only global design tokens, you may prefer to `@use` each settings file then import the library in one. Note the variable names must *lose the module-name prefix* because the module’s scope is clear e.g. `$color-palette-text` is now `$text`.
...
- the variable names must *lose the module-name prefix* because the module’s scope is clear e.g. `$color-palette-text` is now `$text`
