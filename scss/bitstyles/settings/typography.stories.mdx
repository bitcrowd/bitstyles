...
Using a central scale of font-sizes maintains consistency in the typography throughout your project. The default configuration provides base design tokens for font-sizes based on a ratio of 1.17 between each font-size.

These base tokens are used to create a responsive typographic scale that’s applied to the base elements `h1` – `h6` and the corresponding utility classes `.u-h1` – `.u-h6`. The utility classes `.u-text-1` to `.u-text-8` produce exceptions to this responsive typography at specific breakpoints.
...
- [utility classes](/docs/utilities-typography--typography) that apply the same responsive typography as the base elements, for when you need arbitrary elements to consistently match your heading elements
...
These tokens are combined to create the following heading styles.



```scss
/* settings/typography-responsive */
...
- [`typography-responsive`](/docs/utilities-typography-responsive--typography)
...
By overriding `$bitstyles-typography-responsive-font-sizes`, you can specify which font-sizes and line-heights are applied to which HTML elements (in [`base/typography`](/docs/base-typography-responsive-headings--h-1)) and responsive-typography utility classes (in [`utilities/typography-responsive`](/docs/utilities-typography-responsive--typography)), and at which breakpoints.
...
We’re using the global `setup.$no-media-query` to provide font-sizes that apply with no media-query (this is recommended, otherwise you’ll be seeing your browser defaults), and `font-size.get()` to get our font-sizes, `line-height.get()` to get our line-heights from our base design tokens:
