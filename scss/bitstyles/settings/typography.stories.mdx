...
These base tokens are used to create a responsive typographic scale that’s applied to the base elements `h1` – `h6` and the corresponding utility classes `.u-h1` – `.u-h6`, and then utility classes producing exceptions `.u-text-1` to `.u-text-8` at specific breakpoints.
