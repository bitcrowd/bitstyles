...
The exception is the `s` breakpoint. As we’re applying styles always mobile-first, we don’t need a small-or-larger breakpoint — that’s what we have already when we don’t use any breakpoint. The `s` breakpoint instead applies **only** to small viewport sizes, and is there for the exceptional circumstances where applying exceptions for small viewport sizes (instead of vice versa) results in simpler code with fewer classnames or CSS properties.
...
If you need more control, or you want more breakpoints than are present, you’ll need to override the `$breakpoints` map. Note that you cannot add to nor edit the values in the default map, you can only overwrite the entire thing. So be sure to copy the breakpoints you use even if you don’t change them:
