...
If you need more control, or you want more breakpoints than are present, you’ll need to override the `$breakpoints` map. Note that you cannot add to nor edit the values in the default map, you can only overwrite the entire thing, so be sure to copy the breakpoints you’ll use even if you don’t change them:
