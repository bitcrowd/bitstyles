import { Canvas, Meta, Story } from '@storybook/addon-docs/blocks';

<Meta title="Design Tokens/Sizes" />

# Sizes

Using a central scale of sizes maintains consistency in spacing (most often padding and margins) throughout your project. The default sizes are based on font-size to keep the spacing tied to the typography. For users who view your content at a different font-size, the spacing will remain proportional and usable.

The default size scale:

<Canvas>
  <Story name="Sizes">
    {`
      <div class="u-flex u-items-end u-gap-xs">
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-3xs); height: var(--bs-size-3xs)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-2xs); height: var(--bs-size-2xs)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-xs); height: var(--bs-size-xs)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-s); height: var(--bs-size-s)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-m); height: var(--bs-size-m)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-l); height: var(--bs-size-l)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-xl); height: var(--bs-size-xl)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-2xl); height: var(--bs-size-2xl)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-3xl); height: var(--bs-size-3xl)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-4xl); height: var(--bs-size-4xl)"></div>
        <div class="u-shadow-default u-bg-gray-dark u-border-radius-3xs" style="width: var(--bs-size-5xl); height: var(--bs-size-5xl)"></div>
      </div>
    `}
  </Story>
</Canvas>

```scss
$sizes: (
  '0': 0,
  '3xs': 0.125rem, // 2px
  '2xs': 0.25rem, // 4px
  'xs': 0.45rem, // 8px
  's': 0.75rem, // 12px
  'm': 1rem, // 16px
  'l': 1.5rem, // 24px
  'xl': 2rem, // 32px
  '2xl': 2.5rem, // 40px
  '3xl': 3rem, // 48px
  '4xl': 4rem, // 64px
  '5xl': 5rem, // 80px
);
```

## Using the sizes in Sass

Use the `size.get()` helper function for accessing sizes in the sizing scale. In your component Sass file first import the `size` tools, then use the `get()` function, passing the size name:

```scss
@use '~bitstyles/scss/bitstyles/tools/size';

.component-class::after {
  margin-left: size.get('xl');
}
```

With the default setup this will output:

```css
.component-class::after {
  margin-left: 1.6rem;
}
```

### `size.get()` parameters

| Name  | Type   | Description          |
| ----- | ------ | -------------------- |
| $size | string | The name of the size |

## Using the sizes in classes

Use the appropriate utility classes to apply spacing. See the documentation of each class for information on defaults & customization:

- [`u-margin`](/docs/utilities-margin--margin-0)
- [`u-padding`](/docs/utilities-padding--padding-0)

## Using the sizes in CSS

The sizing scale is also available as CSS custom properties. In the default configuration, they are namespaced with `bs`, see [setup](/story/design-tokens-setup--page):

```css
--bs-size-3xs: 0.125rem;
--bs-size-2xs: 0.25rem;
--bs-size-xs: 0.5rem;
--bs-size-s: 0.75rem;
--bs-size-m: 1rem;
--bs-size-l: 1.5rem;
--bs-size-xl: 2rem;
--bs-size-2xl: 2.5rem;
--bs-size-3xl: 3rem;
--bs-size-4xl: 4rem;
--bs-size-5xl: 5rem;
```

## Using size design tokens in JS

The size design tokens can be imported and used in JS:

```js
import sizes from '../exports/_sizes.scss';

console.log(sizes['0']); // '0'
console.log(sizes['2xl']); // '2.5rem'
console.log(sizes['xl']); // '2rem'
console.log(sizes['l']); // '1.5rem'
// …
```

## Customization

### Base size

The size scale is computed from a single starting value. By changing that starting value you can change the whole scale:

```scss
@use '~bitstyles/scss/bitstyles/settings/layout' with (
  $size-base: 2rem
);

/* sizes:

(
  "0": 0,
  "3xs": 0.25rem,
  "2xs": 0.5rem,
  "xs": 1rem,
  "s": 1.5rem,
  "m": 2rem,
  "l": 3rem,
  "xl": 4rem,
  "2xl": 5rem,
  "3xl": 6rem,
  "4xl": 8rem,
  "5xl": 10rem,
)
*/
```

### Overriding the size scale

When you need to increase or reduce the number of sizes available, change the names of the sizes, adjust the ratio between each point of the scale, or otherwise have total control, you’ll need to override the `$sizes` Sass map. Note that you cannot add to nor edit the values in the default map. You can only overwrite the entire thing, so be sure to copy the sizes you’ll use even if you don’t change them.

```scss
@use '~bitstyles/scss/bitstyles/settings/layout' with (
  $sizes: (
    '0': 0,
    // add some pixel-based sizes for small adjustments
    '1': 1px,
    '3xs': 0.125rem,
    '2xs': 0.25rem,
    'xs': 0.5rem,
    's': 0.75rem,
    'm': 1rem,
    'l': 1.25rem,
    'xl': 2rem,
    '2xl': 4rem,
    '3xl': 8rem,
  )
);
```
