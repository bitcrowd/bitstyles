...
Bitstyles provides a semantically-named color palette, with each color having named variants such as `dark`, `light`, `base`. These are used to create the components and the color-related utility classes.

<Story id="base-colors--color-palette" />

## Using the color palette in Sass

Use the `palette.get()` helper function for accessing colors in your Sass. First import `tools/palette`, then use the `get()` function, passing the color name and variant:
...
In the default configuration, these are named with numbers from `5` (darkest) to `195` (lightest) in intervals of 5, with `100` being the base color itself. Due to the way the color adjustment works, color `200` would always be pure white, and color `0` would always be pure black, so they’re not included. That also means that if you want a balanced scale (not necessarily important), the lightness in HSL color space of the base color you provide should be approximately 50%.

The base palette is only used once — during setup to configure our **semantic palette**, which is then used throughout the codebase to apply colors to components, and for any utility classes that apply color. This palette is a limited list of semantically-named colors, selected from the base color palettes, giving colors such as `background-dark`, `danger-base`, `danger-light`.
...
If you have a specific set of colors you need to use, and cannot rely on generated scale to match those, you can skip the base colors and/or the the base palette entirely and just specify your colors in the base and semantic palette.
...
For each color, 39 variants are generated from `5` to `195`. The lower the number, the darker it is.

There are two special cases:

- The `pure-gray` palette offers a scale of 25 colors from black to white, 0 to 100 respectively.
- The `accent-gray` colors are a grayscale mixed with a little accent color (your brand-1 color by default).
...
These are a Sass lists with number values for the percentages of the color to be mixed. The numbers will also be used as the label for the variant. `$color-scale-percentages` is used to generate most colors; `$grayscale-percentages` is used to generate the 0–100 grayscale.
