...
## Default format for design tokens

By default the `get` functions for design tokens (`palette.get`, `size.get`, `font-size.get`, and `line-height.get`) output the name of the CSS custom property for that token. If you prefer the old default behavior of returning the actual value of that token, you can set `$default-token-format` to `value`.

```scss
@use 'bitstyles/settings/setup' with (
  $default-token-format: 'value'
);
```
