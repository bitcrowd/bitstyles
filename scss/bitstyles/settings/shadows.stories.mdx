...
Use `shadow.generate()` to override the global shadow design tokens in `$shadows`:
...
@use '~bitstyles/scss/bitstyles/settings/shadows' with (
  $shadows: (
    'brand-1': shadow.generate($color: rgba(palette.get('brand-1', 'base-rgb'), 0.15), $offset:
          size.get('s7'), $blur-radius: size.get('s4')),
  )
);
```
...
The getter function `shadows.get()` is the most straightforward way to use shadows stored in this format. Specify the name of the shadow and the output format you need. You can use this when configuring the `box-shadow` and `drop-shadow` utility classes, and when using it in your own components. The examples here are for the utility classes:
