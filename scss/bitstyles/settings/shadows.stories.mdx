...
The default shadows used throughout bitstyles are multi-layered to create a more natural or realistic appearance than you get from a single box-shadow or filter. We provide a generator function that you can use to produce these natural-looking multi-layered shadows in the correct format for the Sass design system, and functions to translate those to the box-shadow and drop-shadow formats.

## Generating shadows

Use `shadow.generate()` to override the global shadow design tokens in `$bitstyles-shadows-shadows`:
...
$bitstyles-shadows-shadows: (
  'brand-1': shadow.generate(
      $color: rgba(palette.get('brand-1', 'base-rgb'), 0.15),
      $offset: size.get('s7'),
      $blur-radius: size.get('s4'),
    ),
);
```
...
The accessor function `shadows.get()` is the most straightforward way to use shadows stored in this format. Specify the name of the shadow and the output format you need (these examples are taken from the settings files for `box-shadow` and `drop-shadow` utility classes, each of which uses a different format):
...
The shadow definitions are also available as CSS custom properties. In the default configuration, they are namespaced with `bs`, see [setup](/story/design-tokens-setup--page):

```css
--bs-shadow-brand-1: drop-shadow(rgba(200, 191, 255, 0.125) 0 0.5rem 0.5rem) drop-shadow(
    rgba(200, 191, 255, 0.125) 0 0.6666666667rem 0.6666666667rem
  )
  drop-shadow(rgba(200, 191, 255, 0.125) 0 1rem 1rem) drop-shadow(
    rgba(200, 191, 255, 0.125) 0 2rem 2rem
  );
```
...
console.log(shadows['brand-1']); // 'drop-shadow(rgba(200, 191, 255, 0.125) 0 0.5rem 0.5rem) drop-shadow(rgba(200, 191, 255, 0.125) 0 0.6666666667rem 0.6666666667rem) drop-shadow(rgba(200, 191, 255, 0.125) 0 1rem 1rem) drop-shadow(rgba(200, 191, 255, 0.125) 0 2rem 2rem)'
```
...
For example, take the two shadows overriding the `box-shadow` and `drop-shadow` settings above. We could specify them in-place instead of `shadows.get()`ing them, if they were not part of our design system right now:

```scss
@use '../../tools/shadow';

$bitstyles-box-shadow-values: (
  'brand-1': shadow.to-box-shadow(
      shadow.generate(
        $color: rgba(palette.get('brand-1', 'base-rgb'), 0.15),
        $offset: 2rem,
        $blur-radius: 2rem,
