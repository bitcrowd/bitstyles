import { Canvas, Meta, Story } from '@storybook/addon-docs';

<Meta title="Atoms/Card" />

# Card

A content wrapper that sections and subtly highlights a chunk of content. For larger cards, use the larger `l` size to give the content space to breathe.

<Canvas>
  <Story name="Card">
    {`
    <div class="u-grid u-grid-cols-3@m u-gap-m">
      <article class="a-card">
        <h3 class="u-h4">Standard card</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
      <article class="a-card">
        <h3 class="u-h4">Standard card</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
      <article class="a-card">
        <h3 class="u-h4">Standard card</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
    </div>
    `}
  </Story>
  <Story name="Card-L">
    {`
    <div class="u-grid u-grid-cols-2@m u-gap-m">
      <article class="a-card-l">
        <h3>Large card</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
      <article class="a-card-l">
        <h3>Large card</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
    </div>
    `}
  </Story>
</Canvas>

## Fullbleed headers

It is common to present important content edge-to-edge, using `.a-card__header`:

<Canvas isColumn>
  <Story name="Card header">
    {`
    <div class="u-grid u-grid-cols-2@m u-gap-m">
      <article class="a-card">
        <div class="a-card__header a-flash" data-theme="danger">
          Something happened
        </div>
        <h3 class="u-h4">Card with header</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
    </div>
    `}
  </Story>
  <Story name="Card-L header">
    {`
    <article class="a-card-l">
      <div class="a-card-l__header a-flash" data-theme="danger">
        Something happened
      </div>
      <h3 class="u-h4">Large card with header</h3>
      <ul class="u-list-none">
        <li>User one changed date to 01.01.2021 [5 mins ago]</li>
      </ul>
    </article>
    `}
  </Story>
</Canvas>

## Responsive

The `.a-card` classes are available at `m` and `l` breakpoints by default, including all the sizes of card, so you can apply card styling based on viewport size.

<Canvas>
  <Story name="Card [m breakpoint]">
    {`
      <article class="a-card@m">
        <h3 class="u-h4">Recent activity (a-card@m)</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
    `}
  </Story>
  <Story name="Card [l breakpoint]">
    {`
      <article class="a-card@l">
        <h3 class="u-h4">Recent activity (a-card@l)</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
    `}
  </Story>
  <Story name="Card-L [m breakpoint]">
    {`
      <article class="a-card-l@m">
        <h3 class="u-h4">Recent activity (a-card@m)</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
    `}
  </Story>
  <Story name="Card-L [l breakpoint]">
    {`
      <article class="a-card-l@l">
        <h3 class="u-h4">Recent activity (a-card@l)</h3>
        <ul class="u-list-none">
          <li>User one changed date to 01.01.2021 [5 mins ago]</li>
        </ul>
      </article>
    `}
  </Story>
</Canvas>

## Customization

Card appearance can be customized by overriding the following variables:

```scss
@use '~bitstyles/scss/bitstyles/atoms/card' with (
  $sizes: (
    'base': size.get('m'),
    'l': size.get('l3'),
  ),
  $border-radius: size.get('m'),
  $box-shadow: shadow.to-box-shadow(shadow.generate($color: rgba(palette.get('brand-1', 'base-rgb'), 0.025), $offset:
          size.get('s2'), $blur-radius: size.get('l2'), $layers: 6))
);
```

The breakpoints the cards are available at can be customized by overriding the `$breakpoints` variable:

```scss
@use '~bitstyles/scss/bitstyles/atoms/card' with (
  $breakpoints: (
    'm',
    'l',
  )
);
```
