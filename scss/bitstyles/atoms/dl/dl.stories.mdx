...
Note that the styles will be built mobile-first — if you do not need to override a value from an earlier breakpoint, you do not need to re-sepcify it.
...
There are two extra variables that may be useful, which cannot be specified on a per-breakpoint basis:
...
- `$dt-color` specify what color to use for the `dt` element

```scss
@use '~bitstyles/scss/bitstyles/atoms/dl' with (
  $layout-breakpoint: 'l',
  $dt-color: palette.get('brand-1'),
);
```
