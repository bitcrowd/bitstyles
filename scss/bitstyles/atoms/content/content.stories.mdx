...
<Canvas isColumn>
  <Story name="a-content">
    {`
      <div class="u-bg-grayscale-light-3 u-padding-l2-x">
...
The available content-sizes and their classnames can be customized by overriding the `$max-width` Sass map:
...
The padding that’s applied to all the sizes, and at which breakpoints, can be customized using `$padding`:

```scss
@use '~bitstyles/scss/bitstyles/atoms/content' with (
  $padding: (
    'base': var(design-token.get('size', 'm')),
...
The default size that’s applied when only the base class `a-content` is used defaults to size `m`, but can be set using `$max-width-base`:
