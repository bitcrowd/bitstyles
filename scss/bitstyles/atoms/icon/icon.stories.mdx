import { Canvas, Meta, Story } from '@storybook/addon-docs/blocks';

<Meta title="Atoms/Icon" />

# Icons

## SVG icon system

Icons are implemented using inline SVG where possible (see example markup). They must be accompanied by some machine-readable text for accessibility & SEO purposes. If there is to be no such text visible (e.g. based on design mockups), provide an alternative next to the `<svg>` element using a `<span class="u-sr-only">` containing descriptive text.

The base icon size inherits from the surrounding text i.e. `1em`.

<Canvas isColumn>
  <Story name="Icon">
    {`
      <svg class="a-icon" viewBox="0 0 100 100" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
        <path d="m55.94 1.03v43h43v12h-43v43h-12v-43h-43v-12h43v-43z" fill-rule="evenodd"/>
      </svg>
    `}
  </Story>
  <Story name="Icon with adjacent text">
    {`
      <div class="u-flex u-items-center">
        <svg class="a-icon u-margin-m--right" viewBox="0 0 100 100" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
          <path d="m55.94 1.03v43h43v12h-43v43h-12v-43h-43v-12h43v-43z" fill-rule="evenodd"/>
        </svg>
        <span>This is standard-sized text</span>
      </div>
    `}
  </Story>
</Canvas>

Use the modifier classes `a-icon--<size>` to override this e.g. `.a-icon--s`.

<Canvas>
  <Story name="Small icon">
    {`
      <svg class="a-icon a-icon--s" viewBox="0 0 100 100" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
        <path d="m55.94 1.03v43h43v12h-43v43h-12v-43h-43v-12h43v-43z" fill-rule="evenodd"/>
      </svg>
    `}
  </Story>
  <Story name="Medium icon">
    {`
      <svg class="a-icon a-icon--m" viewBox="0 0 100 100" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
        <path d="m55.94 1.03v43h43v12h-43v43h-12v-43h-43v-12h43v-43z" fill-rule="evenodd"/>
      </svg>
    `}
  </Story>
  <Story name="Large icon">
    {`
      <svg class="a-icon a-icon--l" viewBox="0 0 100 100" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
        <path d="m55.94 1.03v43h43v12h-43v43h-12v-43h-43v-12h43v-43z" fill-rule="evenodd"/>
      </svg>
    `}
  </Story>
</Canvas>
