...
The available values for `flex-direction`, `flex-wrap`, `flex-grow`, and `flex-shrink` and their names can be customized by overriding the `$flex-direction`, `$flex-wrap`, `$flex-grow`, and `$flex-shrink` Sass maps respectively:
...
The breakpoints these classes are available at can be customized by overriding the various `breakpoints` variables `$direction-breakpoints`, `$wrap-breakpoints` etc:
