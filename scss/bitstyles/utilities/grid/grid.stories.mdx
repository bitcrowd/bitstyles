...
For some layouts, you’ll want to allow some of the grid children a little more space. You can do this using `.u-grid__col-span-2` and `.u-grid__col-span-3`, both of which are available `@m`. Note that using these classes can break the grid layout if the `span` class on the child is not applied at the same breakpoint as the grid.
