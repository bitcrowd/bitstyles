...
Override `$bitstyles-typography-values` if you need to provide font-sizes outside of your base font-size design tokens, or a subset of those. But be aware this could lead to inconsistent type treatment — probably you should add the font-size you need to the base font-sizes instead.

```scss
@use '~bitstyles/scss/bitstyles/tools/font-size';
@use 'bitstyles/utilities/typography' with (
  $values: (
    'small': font-size.get('1'),
    'custom': 5.192456rem,
  )
);
```
