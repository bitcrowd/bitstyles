...
Use `shadow.generate()` to override the global shadow design tokens in `$shadows`. Note that when specifying the color, we’re using the `rgb` version as this allows us to specify the opacity in the CSS `rgba` specification (using the hex value would not):
...
@use '~bitstyles/scss/bitstyles/utilities/box-shadows' with (
  $values: (
    'default': design-token.get('shadow', 'brand-1', 'box'),
  )
);
...
@use '~bitstyles/scss/bitstyles/utilities/drop-shadow' with (
  $values: (
    'default': design-token.get('brand-1', 'default', 'drop'),
  )
);
