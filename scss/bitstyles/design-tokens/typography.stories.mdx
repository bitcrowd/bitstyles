...
These design tokens are used to create a responsive typographic scale that’s applied to the base elements `h1` – `h6` and the corresponding utility classes `.u-h1` – `.u-h6` (see the example at the top of this page). The utility classes `.u-text-1` to `.u-text-8` produce exceptions to this responsive typography, at specific breakpoints.
...
### responsive-var(design-token.get('font-size', )

Use the `responsive-var(design-token.get('font-size', )` mixin for accessing the responsive font-sizes specified in your base design tokens. By using this mixin you ensure the font-size of a component will match the responsive typography of your heading elements and responsive-typography utility classes.
...
.component-class::after {
  @include responsive-var(design-token.get('font-size', 'h4');
}
```
...
#### `responsive-var(design-token.get('font-size', )` parameters
...
### var(design-token.get('font-size', )
...
Use the `var(design-token.get('font-size', )` function for accessing the font-sizes specified in your base design tokens. This is useful when you need a component’s font-size to be rendered differently to your standard responsive typography.

Import the `font-size` tools, then use the `get()` function, passing in the font-size you need:
...
`var(design-token.get('font-size', )` is also used in the setup the responsive typographic scale for the [base styles](/docs/base-typography-responsive-headings--h-1) and the [utility classes](/docs/utilities-typography--typography), to ensure consistency across your designs.
...
We’re using the global `setup.$no-media-query` to provide font-sizes that apply with no media-query (this is recommended, otherwise you’ll be seeing your browser defaults), and `var(design-token.get('font-size', )` to get our font-sizes, `line-height.get()` to get our line-heights from our base design tokens:
